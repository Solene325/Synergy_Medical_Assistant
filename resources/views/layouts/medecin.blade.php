...
<head>
    ...
    <title>@yield('title', 'SynergyAI · Médecin')</title>
    ...
    <div class="min-h-screen flex" x-data="{
        ...
        <aside class="sidebar fixed top-0 left-0 h-full z-30 flex flex-col justify-between py-6 px-3">
            ...
            <div>
                ...
                <nav class="flex flex-col gap-1.5">
                    ...
                    <a href="{{ route('medecin.profile.edit') }}" class="nav-item {{ request()->routeIs('medecin.profile.*') ? 'active' : '' }}">
        ...
        <div class="flex-1 ml-[76px] transition-all duration-350 sidebar:hover:ml-[220px]">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div class="flex items-center gap-2 pl-3 border-l border-white/50">
                        <div class="w-9 h-9 rounded-full bg-gradient-to-br from-primary to-primary-light text-white flex items-center justify-center font-bold text-sm shadow-md">
                            {{ strtoupper(substr(auth()->user()->prenom ?? 'M', 0, 1)) }}{{ strtoupper(substr(auth()->user()->nom ?? '', 0, 1)) }}
                        </div>
                        <div class="hidden sm:block text-sm leading-tight">
                            <p class="font-semibold text-primary">Dr. {{ auth()->user()->prenom ?? '' }} {{ auth()->user()->nom ?? '' }}</p>
                            <p class="text-xs text-warm-gray">Médecin</p>
                        </div>
                    </div>
                </div>
            </header>
            ...
            <main class="p-6 lg:p-8">
                @if(session('success'))
                    <div class="p-4 mb-6 bg-soft-green/60 backdrop-blur-sm rounded-2xl border border-green-200/50 text-green-700 text-sm flex items-center gap-2">
                        <i class="fas fa-check-circle text-green-500"></i>
                        {{ session('success') }}
                    </div>
                @endif
                @if($errors->any())
                    <div class="p-4 mb-6 bg-soft-peach/60 backdrop-blur-sm rounded-2xl border border-red-200/50 text-red-700 text-sm flex items-center gap-2">
